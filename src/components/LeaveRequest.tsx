  ...
  export default function LeaveRequest() {
    ...
    const { data: leaveTypes } = useQuery({
      ...
      queryFn: async () => {
        const response = await fetch(
          "http://localhost:8000/api/absence/leaveType"
        );
    ...
    const { data, mutate, isLoading } = useMutation({
      mutationFn: async (data: RequestForm) => {
        await fetch("http://localhost:8000/api/absence/save", {
          method: "POST",
    ...
    const handleSubmit = (event: React.FormEvent) => {
      ...
      const data = { type, date, hour, managerId, managerName, employeeId, employeeFirstName, employeeLastName, team};
      if (hour === 0) {
        toast({
          title: "Алдаа!",
